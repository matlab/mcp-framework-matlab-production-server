...
% Copyright 2025-2026 The MathWorks, Inc.
    ...
    if isempty(fields)  % Nope, not allowed.
        throwAsCaller(MException("prodserver:mcp:EmptyArgImporter", ...
            "Argument import structure may not be empty. Field names " + ...
            "must match input parameter names."));
    end
    ...
    for f = fields
        if isa(x.(f{1}),"matlab.io.ImportOptions") == false
            throwAsCaller(MException("prodserver:mcp:BadArgImporter", ...
                "Invalid type for '%s' import options: '%s'. Import " + ...
                "options must be a matlab.io.ImportOptions object.",...
                f,class(x.(f{1}))));
        end
    end
end
