...
The [prodserver.mcp.feval](../../Documentation/feval.md) function performs these steps according to the MCP protocol. Therefore, if the function returns the expected result, you can be confident that other MCP clients will be able to successfully use this MCP tool.
...
```MATLAB
bpr = prodserver.mcp.feval(endpoint, "primeSequence", 11, "balanced")'
    bpr = 1×17
        5    53   157   173   211   257   263   373   563   593   607   
```
Generate the first 9 Eisenstein primes:
```MATLAB
epr = prodserver.mcp.feval(endpoint, "primeSequence", 9, "eisenstein")'
    epr = 1×9
